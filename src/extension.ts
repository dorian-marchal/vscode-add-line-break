...
const addLineBreak = (direction: "before" | "after") => {
  ...
  const newLinePositions = [] as vscode.Position[];
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("extension.addNewLineBeforeSelection", () =>
      addLineBreak("before")
    )
  );

  context.subscriptions.push(
    vscode.commands.registerCommand("extension.addNewLineAfterSelection", () =>
      addLineBreak("after")
    )
  );
}
